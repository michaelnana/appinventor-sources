...
@DesignerComponent(version = YaVersion.LISTPICKER_COMPONENT_VERSION, category = ComponentCategory.USERINTERFACE, description = "<p>A button that, when clicked on, displays a list of "
    + "texts and/or images for the user to choose among. The texts can be specified through "
    ...
    + "editor.</p> <p>The images can be set using the <code>images</code> property in the Blocks Editor. You can pass a list of URLs to fetch the images for the listpicker or you can use the"
    + " <code>imageForList</code> property of an image to pass to the list.</p> <p> SubElements are alternative text a user might want to display as well for more information. You can set that up "
    + "in the Blocks Editor using the <code>subElement</code> property and passing it a list of strings. </p> "
    + "<p>It's optional to use the <code>Elements</code>, <code>images</code> or <code>subElements</code> properties.</p>"
...
public class ListPicker extends Picker implements ActivityResultListener, Deleteable {
	...
	/**
	 * subElements property getter method
	 * 
	 * @return a YailList representing the list of strings to be picked from
	 */
	@SimpleProperty(category = PropertyCategory.BEHAVIOR)
	public YailList subElements() {
		return subItems;
	}

	/**
	 * Property for sub elements
	 */
	@SimpleProperty
	public void subElements(YailList itemList) {
		Object[] objects = itemList.toStringArray();
		for (int i = 0; i < objects.length; i++) {
			if (!(objects[i] instanceof String)) {
				throw new YailRuntimeError("Items passed to ListPicker must be Strings", "Error");
	 ...
	 * Property for images
	 * @param imageList
	 *          - a YailList containing the images to be added to the ListPicker
	 */
	@SimpleProperty
	public void images(YailList itemList) {
	...
	public Intent getIntent() {
		...
		intent.putExtra(LIST_ACTIVITY_ARG_SUB_NAME, subItems.toStringArray());
		intent.putExtra(LIST_ACTIVITY_ARG_IMG_NAME, images.toStringArray());
